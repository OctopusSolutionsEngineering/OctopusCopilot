# The logic in this file is very lightly modified from ollama_functions.py
...
class LlmFunctions(BaseChatModel):
    llm: LlamaCpp
...
function in "functions".'
        ...
        if not isinstance(chat_generation_content, str):
            raise ValueError("LlmFunctions does not support non-string output.")
        try:
            parsed_chat_result = json.loads(chat_generation_content)
        except json.JSONDecodeError:
            raise ValueError(
                f'llm did not respond with valid JSON. Please try again.'
            )
        ...
        if called_tool is None:
            raise ValueError(
                f"Failed to parse a function call from llm \
