...
from domain.tools.githubactions.cancel_task import cancel_task_callback, cancel_task_confirm_callback_wrapper
...
def build_form_tools(query, req: func.HttpRequest):
    ...
    # tools to handle canceling of a deployment, runbook or task.
    cancel_task_functions = [
        FunctionDefinition(tool, callback=cancel_task_confirm_callback_wrapper(
            get_github_user_from_form(req), url, api_key,
            log_query), is_enabled=is_admin_user(get_github_user_from_form(req), get_admin_users()))
        for tool in cancel_task_wrapper(query, cancel_task_callback(
            url, api_key,
            get_github_user_from_form(req),
            get_functions_connection_string(),
            log_query), log_query)]
