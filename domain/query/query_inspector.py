...
def exclude_all_targets(query, entity_list):
    """
    Determines if the wrapper indicates if all targets should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of targets extracted from the wrapper
...
def exclude_all_runbooks(query, entity_list):
    """
    Determines if the wrapper indicates if all runbooks should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of runbooks extracted from the wrapper
    :return: True if all runbooks should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_runbooks).')
...
def exclude_all_tenants(query, entity_list):
    """
    Determines if the wrapper indicates if all tenants should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of tenants extracted from the wrapper
    :return: True if all tenants should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_tenants).')
...
def exclude_all_projects(query, entity_list):
    """
    Determines if the wrapper indicates if all projects should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of projects extracted from the wrapper
    :return: True if all projects should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_projects).')
...
def exclude_all_library_variable_sets(query, entity_list):
    """
    Determines if the wrapper indicates if all variable sets should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of variable sets extracted from the wrapper
    :return: True if all variable sets should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_library_variable_sets).')
...
def exclude_all_environments(query, entity_list):
    """
    Determines if the wrapper indicates if all environments should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of environments extracted from the wrapper
    :return: True if all environments should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_environments).')
...
def exclude_all_feeds(query, entity_list):
    """
    Determines if the wrapper indicates if all feeds should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of feeds extracted from the wrapper
    :return: True if all feeds should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_feeds).')
...
def exclude_all_accounts(query, entity_list):
    """
    Determines if the wrapper indicates if all accounts should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of accounts extracted from the wrapper
    :return: True if all accounts should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_accounts).')
...
def exclude_all_certificates(query, entity_list):
    """
    Determines if the wrapper indicates if all certificates should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of certificates extracted from the wrapper
    :return: True if all certificates should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_certificates).')
...
def exclude_all_lifecycles(query, entity_list):
    """
    Determines if the wrapper indicates if all lifecycles should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of lifecycles extracted from the wrapper
    :return: True if all lifecycles should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_lifecycles).')
...
def exclude_all_worker_pools(query, entity_list):
    """
    Determines if the wrapper indicates if all worker pools should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of worker pools extracted from the wrapper
    :return: True if all worker pools should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_worker_pools).')
...
def exclude_all_machine_policies(query, entity_list):
    """
    Determines if the wrapper indicates if all machine policies should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of machine policies extracted from the wrapper
    :return: True if all machine policies should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_machine_policies).')
...
def exclude_all_tagsets(query, entity_list):
    """
    Determines if the wrapper indicates if all tag sets should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of tag sets extracted from the wrapper
    :return: True if all tag sets should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_tagsets).')
...
def exclude_all_project_groups(query, entity_list):
    """
    Determines if the wrapper indicates if all project groups should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of project groups extracted from the wrapper
    :return: True if all project groups should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_project_groups).')
...
def exclude_all_steps(query, entity_list):
    """
    Determines if the wrapper indicates if all project steps should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of project steps extracted from the wrapper
    :return: True if all project steps should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_steps).')
...
def exclude_all_variables(query, entity_list):
    """
    Determines if the wrapper indicates if all variables should be excluded.
    :param query: The user's wrapper
    :param entity_list: The list of variables extracted from the wrapper
    :return: True if all variables should be excluded, False otherwise
    """

    ensure_string(query, 'wrapper must be a string (exclude_all_variables).')
...
def release_is_latest(release_version):
    """
    Determines if the wrapper indicates it is talking about the latest release
