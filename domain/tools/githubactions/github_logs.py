...
from infrastructure.github import (
    ...
    get_workflow_run_logs_async,
)
from infrastructure.llm import llm_message_query


def get_github_logs_callback(github_user, github_token, log_query=None):
    def get_github_logs_implementation(
        original_query, messages, owner, repo, workflow, steps, lines
    ):
        ...
        if not workflow:
            return CopilotResponse(
                "You must specify a workflow, for example build.yaml or test.yaml."
            )

        runs = asyncio.run(
            get_latest_workflow_run_async(owner, repo, workflow, github_token)
        ).get("workflow_runs")
        ...
        logs = timing_wrapper(
            lambda: asyncio.run(
                get_workflow_run_logs_async(owner, repo, run_id, github_token)
            ),
            "GitHub Run logs",
        )
