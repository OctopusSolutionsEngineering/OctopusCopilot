...
def releases_query_messages(github_user):
    def releases_query_messages_implementation(
        original_query, space, projects, environments, channels, releases
    ):
        """
        Provide some additional context about the default projects and environments that were used
        to build the list of releases.
        """
        query_project = get_default_argument(
            github_user, get_item_or_none(projects, 0), "Project"
        )
        query_environments = get_default_argument(
            github_user, get_item_or_none(environments, 0), "Environment"
        )
        ...
        if not environments and query_environments:
            if isinstance(query_environments, str):
                additional_messages.append(
                    (
                        "user",
                        f'The question relates to the environment "{query_environments}"',
                    )
                )
            else:
                additional_messages.append(
...
def releases_query_callback(github_user, octopus_details, log_query):
    def releases_query_callback_implementation(
        ...
        query_environments = get_default_argument_list(
            github_user, sanitized_environments, "Environment"
        )

        query_tenants = get_default_argument_list(
            github_user, sanitized_tenants, "Tenant"
        )
