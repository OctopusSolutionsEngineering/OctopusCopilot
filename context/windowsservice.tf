...
variable "project_windows_service_step_deploy_a_windows_service_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy a Windows Service in project Windows Service"
...
resource "octopusdeploy_process_step" "process_step_windows_service_print_message_when_no_targets" {
  ...
  depends_on            = [octopusdeploy_process_step.process_step_windows_service_deploy_a_windows_service]
  properties            = {
      }
  execution_properties  = {
