...
data "octopusdeploy_accounts" "account_fake_aws_oidc" {
  ids          = null
  partial_name = "Fake AWS OIDC"
  skip         = 0
  take         = 1
  account_type = "AmazonWebServicesOidcAccount"
}
resource "octopusdeploy_aws_openid_connect_account" "account_fake_aws_oidc" {
  count                             = "${length(data.octopusdeploy_accounts.account_fake_aws_oidc.accounts) != 0 ? 0 : 1}"
  name                              = "Fake AWS OIDC"
  description                       = "An AWS OIDC account. See https://octopus.com/docs/infrastructure/accounts/aws for more information."
  role_arn                          = "arn:aws:iam::{sourceAccountId}:role/{roleName}"
  account_test_subject_keys         = []
  environments                      = []
  execution_subject_keys            = []
  health_subject_keys               = []
