...
resource "octopusdeploy_process_step" "process_step_aws_lambda_attempt_login" {
  ...
  channels              = null
  condition             = "Success"
  container             = { dockerfile = null, feed_id = "${length(data.octopusdeploy_feeds.feed_ghcr_anonymous.feeds) != 0 ? data.octopusdeploy_feeds.feed_ghcr_anonymous.feeds[0].id : octopusdeploy_docker_container_registry.feed_ghcr_anonymous[0].id}", git_url = null, image = "ghcr.io/octopusdeploylabs/aws-workertools" }
  environments          = null
  excluded_environments = ["${length(data.octopusdeploy_environments.environment_security.environments) != 0 ? data.octopusdeploy_environments.environment_security.environments[0].id : octopusdeploy_environment.environment_security[0].id}"]
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_templated_step.process_step_aws_lambda_octopus___check_smtp_server_configured]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_aws_lambda_validate_setup" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_aws_lambda_attempt_login]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
variable "project_aws_lambda_step_upload_lambda_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Upload Lambda in project AWS Lambda"
...
variable "project_aws_lambda_step_deploy_lambda_sam_template_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy Lambda SAM template in project AWS Lambda"
...
resource "octopusdeploy_process_step" "process_step_aws_lambda_deploy_lambda_sam_template" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_aws_lambda_upload_lambda]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_templated_step" "process_step_aws_lambda_scan_for_vulnerabilities" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_aws_lambda_deploy_lambda_sam_template]
  properties            = {
      }
  execution_properties  = {
        "OctopusUseBundledTooling" = "False"
...
resource "octopusdeploy_process_step" "process_step_aws_lambda_deployment_success_notification" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_aws_lambda_scan_for_vulnerabilities]
  properties            = {
