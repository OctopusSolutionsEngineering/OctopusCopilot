...
resource "octopusdeploy_process_step" "process_step_llm_in_kubernetes_smoke_test" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_llm_in_kubernetes_deploy_a_kubernetes_llm_web_app_via_yaml]
  properties            = {
        "Octopus.Action.TargetRoles" = "Kubernetes"
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_llm_in_kubernetes_print_message_when_no_targets" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Octopus.Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_llm_in_kubernetes_smoke_test]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_llm_in_kubernetes_scan_for_vulnerabilities" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Octopus.Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_llm_in_kubernetes_print_message_when_no_targets]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
