...
resource "octopusdeploy_process_step" "process_step_iis_print_message_when_no_targets" {
  ...
  depends_on            = [octopusdeploy_process_step.process_step_iis_deploy_to_iis]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_iis_scan_for_vulnerabilities" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_iis_deploy_to_iis,octopusdeploy_process_step.process_step_iis_print_message_when_no_targets]
  properties            = {
      }
  execution_properties  = {
