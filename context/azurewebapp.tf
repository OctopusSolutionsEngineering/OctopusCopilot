...
resource "octopusdeploy_deployment_process" "deployment_process_azure_web_app" {
  ...
  step {
    condition            = "Variable"
    condition_expression = "#{unless Octopus.Deployment.Error}#{if Octopus.Action[Check SMTP configuration].Output.SmptConfigured == \"True\"}true#{/if}#{/unless}"
  ...
  step {
    condition            = "Variable"
    condition_expression = "#{if Octopus.Deployment.Error}#{if Octopus.Action[Check SMTP configuration].Output.SmptConfigured == \"True\"}true#{/if}#{/if}"
