...
resource "octopusdeploy_process_step" "process_step_random_quotes__net_iis_approve_production_deployment" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_random_quotes__net_iis_octopus___check_blue_green_deployment]
  properties            = {
      }
  execution_properties  = {
...
variable "project_random_quotes__net_iis_step_transfer_a_package_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Transfer a Package in project Random Quotes .NET IIS"
