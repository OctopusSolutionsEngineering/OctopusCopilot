...
resource "octopusdeploy_process_step" "process_step_terraform_approve_plan" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_terraform_plan_to_apply_a_terraform_template]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_terraform_apply_a_terraform_template" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_terraform_plan_to_apply_a_terraform_template,octopusdeploy_process_step.process_step_terraform_approve_plan]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Terraform.Template" = "#{Project.Terraform.Configuration}"
