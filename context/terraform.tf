...
data "octopusdeploy_projects" "projecttrigger_terraform_daily_security_scan" {
  ids          = null
  partial_name = "Terraform"
  skip         = 0
  take         = 1
}
resource "octopusdeploy_project_scheduled_trigger" "projecttrigger_terraform_daily_security_scan" {
  count       = "${length(data.octopusdeploy_projects.projecttrigger_terraform_daily_security_scan.projects) != 0 ? 0 : 1}"
  space_id    = "${trimspace(var.octopus_space_id)}"
