...
resource "octopusdeploy_process_templated_step" "process_step_kubernetes_web_app_octopus___check_targets_available" {
  count                 = "${length(data.octopusdeploy_projects.project_kubernetes_web_app.projects) != 0 ? 0 : 1}"
  name                  = "Octopus - Check Targets Available"
  process_id            = "${length(data.octopusdeploy_projects.project_kubernetes_web_app.projects) != 0 ? null : octopusdeploy_process.process_kubernetes_web_app[0].id}"
  ...
  parameters            = {
        ...
        "CheckTargets.Octopus.Api.Key" = "#{Octopus.Project.Api.Key}"
...
resource "octopusdeploy_process_step" "process_step_kubernetes_web_app_deploy_a_kubernetes_web_app_via_yaml" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Octopus.Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_kubernetes_web_app_approve_production_deployment,octopusdeploy_process_templated_step.process_step_kubernetes_web_app_octopus___check_targets_available]
  properties            = {
...
resource "octopusdeploy_process_templated_step" "process_step_kubernetes_web_app_scan_for_vulnerabilities" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Octopus.Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_kubernetes_web_app_approve_production_deployment,octopusdeploy_process_templated_step.process_step_kubernetes_web_app_octopus___check_targets_available,octopusdeploy_process_step.process_step_kubernetes_web_app_deploy_a_kubernetes_web_app_via_yaml]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
