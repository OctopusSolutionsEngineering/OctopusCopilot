...
resource "octopusdeploy_process_templated_step" "process_step_octopub_octopus___check_smtp_server_configured" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_templated_step.process_step_octopub_octopus___check_targets_available]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
variable "project_octopub_step_deploy_to_tomcat_via_manager_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy to Tomcat via Manager in project Octopub"
...
resource "octopusdeploy_process_templated_step" "process_step_octopub_scan_for_vulnerabilities" {
  ...
  tenant_tags           = null
  worker_pool_variable  = "Project.WorkerPool"
  depends_on            = [octopusdeploy_process_step.process_step_octopub_deploy_to_tomcat_via_manager]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_octopub_deployment_success_notification" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_octopub_scan_for_vulnerabilities]
  properties            = {
