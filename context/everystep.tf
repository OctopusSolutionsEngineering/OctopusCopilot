...
resource "octopusdeploy_environment" "environment_green_production" {
  ...
  depends_on = [octopusdeploy_environment.environment_development,octopusdeploy_environment.environment_test,octopusdeploy_environment.environment_production,octopusdeploy_environment.environment_blue_production,octopusdeploy_environment.environment_security]
  lifecycle {
    prevent_destroy = true
  }
}

data "octopusdeploy_lifecycles" "lifecycle_blue_green_lifecycle" {
  ids          = null
  partial_name = "Blue/Green Lifecycle"
  skip         = 0
  take         = 1
}
resource "octopusdeploy_lifecycle" "lifecycle_blue_green_lifecycle" {
  count       = "${length(data.octopusdeploy_lifecycles.lifecycle_blue_green_lifecycle.lifecycles) != 0 ? 0 : 1}"
  name        = "Blue/Green Lifecycle"
  description = ""
...
data "octopusdeploy_projects" "channel_every_step_project_ephemeral_environments" {
  ids          = null
  partial_name = "Every Step Project"
  skip         = 0
  take         = 1
}
resource "octopusdeploy_channel" "channel_every_step_project_ephemeral_environments" {
  count                               = "${length(data.octopusdeploy_projects.channel_every_step_project_ephemeral_environments.projects) != 0 ? 0 : 1}"
  ephemeral_environment_name_template = "#{Octopus.Release.CustomFields[FeatureBranch]}"
  parent_environment_id               = "${length(data.octopusdeploy_parent_environments.parent_environment_features.parent_environments) != 0 ? data.octopusdeploy_parent_environments.parent_environment_features.parent_environments[0].id : octopusdeploy_parent_environment.parent_environment_features[0].id}"
...
data "octopusdeploy_community_step_template" "communitysteptemplate_octopus___prompt_ai" {
  website = "https://library.octopus.com/step-templates/8ce3eb55-2c35-45c2-be8c-27e71ffbf032"
}
data "octopusdeploy_step_template" "steptemplate_octopus___prompt_ai" {
  name = "Octopus - Prompt AI"
}
resource "octopusdeploy_community_step_template" "communitysteptemplate_octopus___prompt_ai" {
  community_action_template_id = "${length(data.octopusdeploy_community_step_template.communitysteptemplate_octopus___prompt_ai.steps) != 0 ? data.octopusdeploy_community_step_template.communitysteptemplate_octopus___prompt_ai.steps[0].id : null}"
  count                        = "${data.octopusdeploy_step_template.steptemplate_octopus___prompt_ai.step_template != null ? 0 : 1}"
}
...
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_octopus___check_blue_green_deployment" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Octopus - Check Blue Green Deployment"
  ...
  excluded_environments = null
  notes                 = "This step is the first of two that implement Blue/Green deployments in Octopus. When a prompt includes instructions to implement Blue/Green deployments:\n\n* This step must be the first step in the deployment process\n* The \"Blue Production\" and \"Green Production\" environments must be created\n* The step is run only in the \"Blue Production\" and \"Green Production\" environments\n* The \"Production\" environment must be removed\n* Those environments must be placed in a lifecycle with the phase called \"Production\"\n* Add the \"Proceed with Blue Green deployment\" manual intervention step\n\nThese instructions must be considered in addition to any instructions to create steps from example projects.\n\nThis description is only intended to guide the LLM when generating projects. The description MUST NOT be included in any generated steps."
  package_requirement   = "LetOctopusDecide"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_proceed_with_blue_green_deployment" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Proceed with Blue Green deployment"
  type                  = "Octopus.Manual"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Variable"
  environments          = ["${length(data.octopusdeploy_environments.environment_blue_production.environments) != 0 ? data.octopusdeploy_environments.environment_blue_production.environments[0].id : octopusdeploy_environment.environment_blue_production[0].id}", "${length(data.octopusdeploy_environments.environment_green_production.environments) != 0 ? data.octopusdeploy_environments.environment_green_production.environments[0].id : octopusdeploy_environment.environment_green_production[0].id}"]
  excluded_environments = null
  notes                 = "This is the second step that makes up a blue/green deployment.\n\nThis manual intervention is displayed only when the `SequentialDeploy `output variable is `True`. It indicates that the deployment does not follow the traditional Blue/Green sequence, and requests approval.\n\nThis step MUST be included when the \"Octopus - Check Blue Green Deployment\" step is added to a deployment process."
  package_requirement   = "LetOctopusDecide"
  slug                  = "proceed-with-blue-green-deployment"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_octopus___check_blue_green_deployment]
  properties            = {
        "Octopus.Step.ConditionVariableExpression" = "#{Octopus.Action[Octopus - Check Blue Green Deployment].Output.SequentialDeploy}"
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_a_script" {
  ...
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_proceed_with_blue_green_deployment]
  properties            = {
      }
  execution_properties  = {
        "OctopusUseBundledTooling" = "False"
...
variable "project_every_step_project_step_run_a_script_from_a_package_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Run a Script from a package in project Every Step Project"
  default     = "scripts"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_run_a_script_from_a_package" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_run_a_script_from_a_package_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "run-a-script-from-a-package-1"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = ["Tag Set/tag", "Tag Set/tag2"]
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_a_script]
  properties            = {
      }
  execution_properties  = {
        "OctopusUseBundledTooling" = "False"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_an_azure_script" {
  ...
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_a_script_from_a_package]
  properties            = {
      }
  execution_properties  = {
...
variable "project_every_step_project_step_run_an_azure_script_from_a_package_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Run an Azure Script from a package in project Every Step Project"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_an_azure_script_from_a_package" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_run_an_azure_script_from_a_package_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "run-an-azure-script-from-a-package"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = ["Business Units/Billing", "Business Units/Engineering", "Business Units/HR", "Business Units/Insurance", "Cities/London", "Cities/Madrid", "Cities/Sydney", "Cities/Washington", "Cities/Wellington", "Regions/ANZ", "Regions/Asia", "Regions/Europe", "Regions/US"]
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_an_azure_script]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_an_aws_cli_script" {
  ...
  channels              = null
  condition             = "Success"
  container             = { dockerfile = null, feed_id = "${length(data.octopusdeploy_feeds.feed_github_container_registry.feeds) != 0 ? data.octopusdeploy_feeds.feed_github_container_registry.feeds[0].id : octopusdeploy_docker_container_registry.feed_github_container_registry[0].id}", git_url = null, image = "ghcr.io/octopusdeploylabs/aws-workertools" }
  environments          = null
  excluded_environments = null
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_an_azure_script_from_a_package]
  properties            = {
      }
  execution_properties  = {
...
variable "project_every_step_project_step_run_an_aws_cli_script_from_package_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Run an AWS CLI Script from package in project Every Step Project"
  default     = "awscript"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_run_an_aws_cli_script_from_package" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_run_an_aws_cli_script_from_package_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "run-an-aws-cli-script-from-package"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_an_aws_cli_script]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_gcloud_in_a_script" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_an_aws_cli_script_from_package]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_gcloud_in_a_script_with_an_account" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_gcloud_in_a_script]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Script.ScriptBody" = "echo \"Hi\""
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_an_azure_resource_manager_template" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_gcloud_in_a_script_with_an_account]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Azure.AccountId" = "${length(data.octopusdeploy_accounts.account_azure.accounts) != 0 ? data.octopusdeploy_accounts.account_azure.accounts[0].id : octopusdeploy_azure_openid_connect.account_azure[0].id}"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_a_kubectl_script" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_an_azure_resource_manager_template]
  properties            = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_a_bicep_template" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Deploy a Bicep template"
  type                  = "Octopus.AzureDeployBicepTemplate"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  container             = { dockerfile = null, feed_id = "${length(data.octopusdeploy_feeds.feed_github_container_registry.feeds) != 0 ? data.octopusdeploy_feeds.feed_github_container_registry.feeds[0].id : octopusdeploy_docker_container_registry.feed_github_container_registry[0].id}", git_url = null, image = "ghcr.io/octopusdeploylabs/azure-workertools" }
  environments          = null
  excluded_environments = null
  notes                 = "Deploys an Azure Bicep template."
  package_requirement   = "LetOctopusDecide"
  slug                  = "deploy-a-bicep-template"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_a_kubectl_script]
  properties            = {
      }
  execution_properties  = {
...
variable "project_every_step_project_step_deploy_to_iis_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy to IIS in project Every Step Project"
  default     = "webapp"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_to_iis" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_to_iis_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-to-iis"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_a_bicep_template]
  properties            = {
...
variable "project_every_step_project_step_deploy_a_package_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy a Package in project Every Step Project"
  default     = "mypackage"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_a_package" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_a_package_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-a-package"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_to_iis]
  properties            = {
        "Octopus.Action.TargetRoles" = "windows-server"
      }
  execution_properties  = {
...
variable "project_every_step_project_step_deploy_a_windows_service_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy a Windows Service in project Every Step Project"
  default     = "myservice"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_a_windows_service" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_a_windows_service_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-a-windows-service"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_a_package]
  properties            = {
        "Octopus.Action.TargetRoles" = "windows-server"
      }
  execution_properties  = {
...
variable "project_every_step_project_step_deploy_an_azure_web_app__web_deploy__packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy an Azure Web App (Web Deploy) in project Every Step Project"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_an_azure_web_app__web_deploy_" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_an_azure_web_app__web_deploy__packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-an-azure-web-app-web-deploy"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_a_windows_service]
  properties            = {
...
variable "project_every_step_project_step_upload_a_package_to_an_aws_s3_bucket_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Upload a package to an AWS S3 bucket in project Every Step Project"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_upload_a_package_to_an_aws_s3_bucket" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_upload_a_package_to_an_aws_s3_bucket_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "upload-a-package-to-an-aws-s3-bucket"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_an_azure_web_app__web_deploy_]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.AwsAccount.Variable" = "Account.AWS"
...
variable "project_every_step_project_step_deploy_java_archive_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy Java Archive in project Every Step Project"
  default     = "MyJavaApp"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_java_archive" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_java_archive_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-java-archive"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_upload_a_package_to_an_aws_s3_bucket]
  properties            = {
...
variable "project_every_step_project_step_deploy_a_helm_chart_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy a Helm Chart in project Every Step Project"
  default     = "MyHelmApp"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_a_helm_chart" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${length(data.octopusdeploy_feeds.feed_helm.feeds) != 0 ? data.octopusdeploy_feeds.feed_helm.feeds[0].id : octopusdeploy_helm_feed.feed_helm[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_a_helm_chart_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-a-helm-chart"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_java_archive]
  properties            = {
        "Octopus.Action.TargetRoles" = "Kubernetes"
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_kubernetes_yaml" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_a_helm_chart]
  properties            = {
        "Octopus.Action.TargetRoles" = "Kubernetes"
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_kubernetes_yaml_with_client_side_apply" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_kubernetes_yaml]
  properties            = {
        "Octopus.Action.TargetRoles" = "Kubernetes"
      }
  execution_properties  = {
...
variable "project_every_step_project_step_deploy_kubernetes_yaml_from_a_package_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy Kubernetes YAML from a package in project Every Step Project"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_kubernetes_yaml_from_a_package" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_kubernetes_yaml_from_a_package_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-kubernetes-yaml-from-a-package"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_kubernetes_yaml_with_client_side_apply]
  properties            = {
        "Octopus.Action.TargetRoles" = "Kubernetes"
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_with_kustomize" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_kubernetes_yaml_from_a_package]
  properties            = {
        "Octopus.Action.TargetRoles" = "Kubernetes"
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_apply_a_terraform_template" {
  ...
  channels              = null
  condition             = "Success"
  container             = { dockerfile = null, feed_id = "${length(data.octopusdeploy_feeds.feed_github_container_registry.feeds) != 0 ? data.octopusdeploy_feeds.feed_github_container_registry.feeds[0].id : octopusdeploy_docker_container_registry.feed_github_container_registry[0].id}", git_url = null, image = "ghcr.io/octopusdeploylabs/terraform-workertools" }
  environments          = null
  ...
  notes                 = "This step deploys a Terraform configuration file.\n\nTerraform output variables are exposed as Octopus Output variables\n\nSimple values are exposed as:\n\n`Octopus.Action[Apply a Terraform template].Output.TerraformValueOutputs[simplevalue]`\n\nJSON values are exposed as:\n\n`Octopus.Action[Apply a Terraform template].Output.TerraformJsonOutputs[simplevalue]`\n\nThe JSON value looks like:\n\n```\n{ \n  \"sensitive\": false, \n  \"type\": \"string\", \n  \"value\": \"hi there\" \n}\n```\n\nYou MUST create the `Octopus.Action.Terraform.TemplateParameters` key in the `execution_properties`. If it is empty, define it as `{}`."
  package_requirement   = "LetOctopusDecide"
  slug                  = "apply-a-terraform-template"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_with_kustomize]
  properties            = {
      }
  execution_properties  = {
        "OctopusUseBundledTooling" = "False"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_destroy_terraform_resources" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_apply_a_terraform_template]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Terraform.AzureAccount" = "False"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_plan_to_apply_a_terraform_template" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_destroy_terraform_resources]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Terraform.PlanJsonOutput" = "False"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_plan_a_terraform_destroy" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_plan_to_apply_a_terraform_template]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Terraform.AzureAccount" = "False"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_create_an_amazon_s3_bucket" {
  ...
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_plan_a_terraform_destroy]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_an_aws_cloudformation_template" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_create_an_amazon_s3_bucket]
  properties            = {
...
variable "project_every_step_project_step_deploy_an_aws_cloudformation_template_from_package_packageid" {
  ...
  nullable    = false
  sensitive   = false
  description = "The package ID for the default package from step Deploy an AWS CloudFormation template from package in project Every Step Project"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_an_aws_cloudformation_template_from_package" {
  ...
  package_requirement   = "LetOctopusDecide"
  primary_package       = { acquisition_location = "Server", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_deploy_an_aws_cloudformation_template_from_package_packageid}", properties = { SelectionMode = "immediate" }, version = null }
  slug                  = "deploy-an-aws-cloudformation-template-from-package"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_an_aws_cloudformation_template]
  properties            = {
        "Octopus.Step.ConditionVariableExpression" = "#{Step.Run}"
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_apply_an_aws_cloudformation_change_set" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_an_aws_cloudformation_template_from_package]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_delete_an_aws_cloudformation_stack" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_apply_an_aws_cloudformation_change_set]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.AwsAccount.UseInstanceRole" = "False"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_a_script_on_failure" {
  ...
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_delete_an_aws_cloudformation_stack]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Script.ScriptBody" = "echo \"hi\""
...
resource "octopusdeploy_process_step" "process_step_every_step_project_a_step_with_a_conditionvariableexpression" {
  ...
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_a_script_on_failure]
  properties            = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_a_release___deploy_release_if_new_deployment_is_higher" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_a_step_with_a_conditionvariableexpression]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_a_release___always_deploy" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_a_release___deploy_release_if_new_deployment_is_higher]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.DeployRelease.ProjectId" = "${length(data.octopusdeploy_projects.project_child_project.projects) != 0 ? data.octopusdeploy_projects.project_child_project.projects[0].id : octopusdeploy_project.project_child_project[0].id}"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_a_release___always_deploy_" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_a_release___always_deploy]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_readyroll___deploy_database_package" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_a_release___always_deploy_]
  properties            = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_a_script_for_a_tenant" {
  ...
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_windows.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_readyroll___deploy_database_package]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_an_aws_cli_script_with_retries_enabled" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_a_script_for_a_tenant]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Aws.AssumeRole" = "False"
...
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_slack___send_simple_notification" {
  ...
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_an_aws_cli_script_with_retries_enabled]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_deploy_kubernetes_yaml_from_git_repo" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Deploy Kubernetes YAML from Git Repo"
  type                  = "Octopus.KubernetesDeployRawYaml"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  environments          = null
  excluded_environments = null
  git_dependencies      = { "" = { default_branch = "main", file_path_filters = null, git_credential_id = "${length(data.octopusdeploy_git_credentials.gitcredential_github.git_credentials) != 0 ? data.octopusdeploy_git_credentials.gitcredential_github.git_credentials[0].id : octopusdeploy_git_credential.gitcredential_github[0].id}", git_credential_type = "Library", github_connection_id = "", repository_uri = "https://example.invalid/url-0978" } }
  package_requirement   = "LetOctopusDecide"
  slug                  = "deploy-kubernetes-yaml-from-git-repo"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_slack___send_simple_notification]
  properties            = {
        "Octopus.Action.TargetRoles" = "Kubernetes"
      }
  execution_properties  = {
...
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_octopus___prompt_ai" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Octopus - Prompt AI"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  template_id           = "${data.octopusdeploy_step_template.steptemplate_octopus___prompt_ai.step_template != null ? data.octopusdeploy_step_template.steptemplate_octopus___prompt_ai.step_template.id : octopusdeploy_community_step_template.communitysteptemplate_octopus___prompt_ai[0].id}"
  template_version      = "${data.octopusdeploy_step_template.steptemplate_octopus___prompt_ai.step_template != null ? data.octopusdeploy_step_template.steptemplate_octopus___prompt_ai.step_template.version : octopusdeploy_community_step_template.communitysteptemplate_octopus___prompt_ai[0].version}"
  channels              = null
  condition             = "Success"
  environments          = null
  excluded_environments = null
  notes                 = "This community step template has the url https://library.octopus.com/step-templates/8ce3eb55-2c35-45c2-be8c-27e71ffbf032/actiontemplate-octopus-prompt-ai"
  package_requirement   = "LetOctopusDecide"
  slug                  = "octopus-prompt-ai"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_deploy_kubernetes_yaml_from_git_repo]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_apply_a_terraform_template_with_azure" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Apply a Terraform template with Azure"
  type                  = "Octopus.TerraformApply"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  ...
  excluded_environments = null
  package_requirement   = "LetOctopusDecide"
  slug                  = "apply-a-terraform-template-with-azure"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_octopus___prompt_ai]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Terraform.PlanJsonOutput" = "False"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_apply_a_terraform_template_with_aws" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Apply a Terraform template with AWS"
  type                  = "Octopus.TerraformApply"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  ...
  excluded_environments = null
  package_requirement   = "LetOctopusDecide"
  slug                  = "apply-a-terraform-template-with-aws"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_apply_a_terraform_template_with_azure]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Terraform.AllowPluginDownloads" = "True"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_apply_a_terraform_template_with_google" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Apply a Terraform template with Google"
  type                  = "Octopus.TerraformApply"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  ...
  excluded_environments = null
  package_requirement   = "LetOctopusDecide"
  slug                  = "apply-a-terraform-template-with-google"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_apply_a_terraform_template_with_aws]
  properties            = {
      }
  execution_properties  = {
...
resource "google_storage_bucket" "my_bucket" {
  ...
  location      = "US"                          # Multi-region or specific region
  storage_class = "STANDARD"

  # Optional: Prevents accidental deletion of the bucket
  force_destroy = false

  # Optional: Recommended for modern security (IAM-only access)
  uniform_bucket_level_access = true

  # Optional: Enable versioning to keep old versions of objects
  versioning {
    enabled = true
  }
}
...
resource "octopusdeploy_process_step" "process_step_every_step_project_update_argo_cd_application_manifests__additional_options_" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Update Argo CD Application Manifests (additional options)"
  ...
  environments          = null
  excluded_environments = null
  git_dependencies      = { "" = { default_branch = "main", file_path_filters = null, git_credential_id = "${length(data.octopusdeploy_git_credentials.gitcredential_mock.git_credentials) != 0 ? data.octopusdeploy_git_credentials.gitcredential_mock.git_credentials[0].id : octopusdeploy_git_credential.gitcredential_mock[0].id}", git_credential_type = "Library", github_connection_id = "", repository_uri = "https://mockgit.octopusdemos.com" } }
  is_disabled           = true
  notes                 = "Demonstrates additional options for the  Update Argo CD Application Manifests step."
  package_requirement   = "LetOctopusDecide"
  slug                  = "update-argo-cd-application-manifests"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_apply_a_terraform_template_with_google]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_step" "process_step_every_step_project_update_argo_cd_application_manifests___clone__1_" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Update Argo CD Application Manifests - clone (1)"
  type                  = "Octopus.ArgoCDUpdateManifests"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  ...
  git_dependencies      = { "" = { default_branch = "main", file_path_filters = null, git_credential_id = "${length(data.octopusdeploy_git_credentials.gitcredential_mock.git_credentials) != 0 ? data.octopusdeploy_git_credentials.gitcredential_mock.git_credentials[0].id : octopusdeploy_git_credential.gitcredential_mock[0].id}", git_credential_type = "Library", github_connection_id = "", repository_uri = "https://mockgit.octopusdemos.com" } }
  package_requirement   = "LetOctopusDecide"
  slug                  = "update-argo-cd-application-manifests-clone-1"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_update_argo_cd_application_manifests__additional_options_]
  properties            = {
      }
  execution_properties  = {
...
variable "project_every_step_project_step_update_argo_cd_application_image_tags_package_nginx_packageid" {
  type        = string
  nullable    = false
  sensitive   = false
  description = "The package ID for the package named nginx from step Update Argo CD Application Image Tags in project Every Step Project"
  default     = "nginx"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_update_argo_cd_application_image_tags" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  ...
  is_disabled           = true
  package_requirement   = "LetOctopusDecide"
  packages              = { nginx = { acquisition_location = "NotAcquired", feed_id = "${length(data.octopusdeploy_feeds.feed_docker_hub.feeds) != 0 ? data.octopusdeploy_feeds.feed_docker_hub.feeds[0].id : octopusdeploy_docker_container_registry.feed_docker_hub[0].id}", id = null, package_id = "${var.project_every_step_project_step_update_argo_cd_application_image_tags_package_nginx_packageid}", properties = { Extract = "False", Purpose = "DockerImageReference", SelectionMode = "immediate" }, version = null } }
  slug                  = "update-argo-cd-application-image-tags"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_update_argo_cd_application_manifests___clone__1_]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_wait_for_argo_cd_applications" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Wait for Argo CD Applications"
  type                  = "Octopus.VerifyArgoCDApplicationHealthy"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  environments          = null
  excluded_environments = null
  package_requirement   = "LetOctopusDecide"
  slug                  = "wait-for-argo-cd-applications"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_update_argo_cd_application_image_tags]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.ArgoCD.StepVerification.Method" = "ArgoCDApplicationExists"
        "Octopus.Action.ArgoCD.WaitForApps.LabelFilter" = "team: payments"
        "Octopus.Action.ArgoCD.WaitForApps.ExpectedCommitHashes" = "a2c5e69b2d87e07a2139bc345d1f2e8b9a10c43d,e18f72a4c9b13054817a0b234567c9d08e1a4f3b, 9f2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b"
        "Octopus.Action.ArgoCD.WaitForApps.ExpectedNumberOfApplications" = "1"
        "Octopus.Action.ArgoCD.StepVerification.Timeout" = "180"
      }
}

variable "project_every_step_project_step_commit_to_git_package_inputpack_1_packageid" {
  type        = string
  nullable    = false
  sensitive   = false
  description = "The package ID for the package named InputPack-1 from step Commit to Git in project Every Step Project"
  default     = "test2"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_commit_to_git" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Commit to Git"
  type                  = "Octopus.CommitToGit"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  environments          = null
  excluded_environments = null
  package_requirement   = "LetOctopusDecide"
  packages              = { InputPack-1 = { acquisition_location = "ExecutionTarget", feed_id = "${data.octopusdeploy_feeds.feed_octopus_server__built_in_.feeds[0].id}", id = null, package_id = "${var.project_every_step_project_step_commit_to_git_package_inputpack_1_packageid}", properties = { Extract = "true", SelectionMode = "immediate" }, version = null } }
  slug                  = "commit-to-git"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_wait_for_argo_cd_applications]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_calculate_deployment_mode" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Calculate Deployment Mode"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  ...
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_commit_to_git]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
variable "action_43d4bc183f270365807d235f0834be5233ca1ad0f4714d0bf7238010ae3cb8a9_sensitive_value" {
  type        = string
  nullable    = false
  sensitive   = true
  description = "Sensitive value for property Octopus.Action.Claude.ApiKey"
  default     = "Change Me!"
}
variable "action_acab4a1ca95fc8bef952eeee52a48af2fbc460bf7e0f39616ad01a4e70605921_sensitive_value" {
  type        = string
  nullable    = false
  sensitive   = true
  description = "Sensitive value for property Octopus.Action.Claude.OctopusMcpApiKey"
  default     = "Change Me!"
}
resource "octopusdeploy_process_step" "process_step_every_step_project_run_claude_agent" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  ...
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_calculate_deployment_mode]
  properties            = {
      }
  execution_properties  = {
...
variable "action_37116d510d53bfaca6c6c67f1be91da835a12fda21349d86acd887a32c09d21f_sensitive_value" {
  type        = string
  nullable    = false
  sensitive   = true
  description = "Sensitive value for property Run.Runbook.Api.Key"
  default     = "Change Me!"
}
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_run_octopus_deploy_runbook" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Run Octopus Deploy Runbook"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  ...
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_claude_agent]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
variable "action_c8ebbe330969c87f860654a448eb873b4f1c05ec542b95f7e80476a329266a47_sensitive_value" {
  type        = string
  nullable    = false
  sensitive   = true
  description = "Sensitive value for property Block.Octopus.Api.Key"
  default     = "Change Me!"
}
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_block_release_progression" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Block Release Progression"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  ...
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_run_octopus_deploy_runbook]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_apply_a_terraform_template_with_empty_parameters" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Apply a Terraform template with empty parameters"
  type                  = "Octopus.TerraformApply"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  ...
  excluded_environments = null
  package_requirement   = "LetOctopusDecide"
  slug                  = "apply-a-terraform-template-with-empty-parameters"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_block_release_progression]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.Terraform.PlanJsonOutput" = "False"
...
variable "action_1952dd9e318cf92357ea967f90bb7290a8b40af88806509ec708b770af7ca2e9_sensitive_value" {
  ...
  nullable    = false
  sensitive   = true
  description = "Sensitive value for property SelfSupport.Claude.ApiKey"
  default     = "Change Me!"
}
variable "action_e736b8984f9155db6c8b165e9aced40baaf960179420238bf57d4b18a8a3b137_sensitive_value" {
  type        = string
  nullable    = false
  sensitive   = true
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_a_process_template" {
  ...
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_apply_a_terraform_template_with_empty_parameters]
  properties            = {
      }
  execution_properties  = {
...
resource "octopusdeploy_process_templated_step" "process_step_every_step_project_automate_manual_intervention_response" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Automate Manual Intervention Response"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  template_id           = "${data.octopusdeploy_step_template.steptemplate_automate_manual_intervention_response.step_template != null ? data.octopusdeploy_step_template.steptemplate_automate_manual_intervention_response.step_template.id : octopusdeploy_community_step_template.communitysteptemplate_automate_manual_intervention_response[0].id}"
  template_version      = "${data.octopusdeploy_step_template.steptemplate_automate_manual_intervention_response.step_template != null ? data.octopusdeploy_step_template.steptemplate_automate_manual_intervention_response.step_template.version : octopusdeploy_community_step_template.communitysteptemplate_automate_manual_intervention_response[0].version}"
  channels              = null
  condition             = "Success"
  environments          = null
  excluded_environments = null
  package_requirement   = "LetOctopusDecide"
  slug                  = "automate-manual-intervention-response"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  worker_pool_id        = "${length(data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools) != 0 ? data.octopusdeploy_worker_pools.workerpool_hosted_ubuntu.worker_pools[0].id : data.octopusdeploy_worker_pools.workerpool_default_worker_pool.worker_pools[0].id}"
  depends_on            = [octopusdeploy_process_step.process_step_every_step_project_run_a_process_template]
  properties            = {
      }
  execution_properties  = {
        "Octopus.Action.RunOnServer" = "true"
...
resource "octopusdeploy_process_step" "process_step_every_step_project_run_a_script_on_target" {
  count                 = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  name                  = "Run a Script on target"
  type                  = "Octopus.Script"
  process_id            = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_process.process_every_step_project[0].id}"
  channels              = null
  condition             = "Success"
  environments          = null
  excluded_environments = null
  notes                 = "This is an example of a script step run on a target.\n\nNote \"Octopus.Action.RunOnServer\" is set to false when \"Octopus.Action.TargetRoles\" is set to a target tag."
  package_requirement   = "LetOctopusDecide"
  slug                  = "run-a-script-on-target"
  start_trigger         = "StartAfterPrevious"
  tenant_tags           = null
  depends_on            = [octopusdeploy_process_templated_step.process_step_every_step_project_automate_manual_intervention_response]
  properties            = {
...
resource "octopusdeploy_variable" "every_step_project_runbook_scoped_variable_1" {
  ...
  scope {
    ...
    roles        = null
    tenant_tags  = null
    processes    = ["${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_runbook.runbook_every_step_project_example_runbook[0].id}"]
  }
  lifecycle {
    ignore_changes  = [sensitive_value]
    prevent_destroy = true
...
resource "octopusdeploy_variable" "every_step_project_project_octopus_api_key_1" {
  count           = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  owner_id        = "${length(data.octopusdeploy_projects.project_every_step_project.projects) == 0 ?octopusdeploy_project.project_every_step_project[0].id : data.octopusdeploy_projects.project_every_step_project.projects[0].id}"
...
resource "octopusdeploy_variable" "every_step_project_project_release_id_1" {
  count        = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  owner_id     = "${length(data.octopusdeploy_projects.project_every_step_project.projects) == 0 ?octopusdeploy_project.project_every_step_project[0].id : data.octopusdeploy_projects.project_every_step_project.projects[0].id}"
...
resource "octopusdeploy_variable" "every_step_project_runbookvariable_1" {
  count        = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  owner_id     = "${length(data.octopusdeploy_projects.project_every_step_project.projects) == 0 ?octopusdeploy_project.project_every_step_project[0].id : data.octopusdeploy_projects.project_every_step_project.projects[0].id}"
  name         = "RunbookVariable"
  type         = "String"
  is_sensitive = false
...
resource "octopusdeploy_project_scheduled_trigger" "projecttrigger_every_step_project_create_new_release" {
  count       = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  space_id    = "${trimspace(var.octopus_space_id)}"
  name        = "Create new release"
  description = "This is an example of a trigger that creates and deploys a new release."
  timezone    = "UTC"
  is_disabled = false
  project_id  = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? data.octopusdeploy_projects.project_every_step_project.projects[0].id : octopusdeploy_project.project_every_step_project[0].id}"
  tenant_ids  = []
...
resource "octopusdeploy_webhook_trigger" "projecttrigger_every_step_project_runbook_webhook_trigger" {
  count              = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  space_id           = "${trimspace(var.octopus_space_id)}"
  name               = "Runbook Webhook Trigger"
  description        = "This is an example of the runbook webhook trigger. This example requires an Octopus API key to trigger."
  project_id         = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? data.octopusdeploy_projects.project_every_step_project.projects[0].id : octopusdeploy_project.project_every_step_project[0].id}"
  require_api_key    = true
  tenant_ids         = ["${length(data.octopusdeploy_tenants.tenant_australian_office.tenants) != 0 ? data.octopusdeploy_tenants.tenant_australian_office.tenants[0].id : octopusdeploy_tenant.tenant_australian_office[0].id}"]
  run_runbook_action = { runbook_id = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_runbook.runbook_every_step_project_example_runbook[0].id}", target_environment_ids = ["${length(data.octopusdeploy_environments.environment_development.environments) != 0 ? data.octopusdeploy_environments.environment_development.environments[0].id : octopusdeploy_environment.environment_development[0].id}"] }
  lifecycle {
    prevent_destroy = true
  }
}

resource "octopusdeploy_webhook_trigger" "projecttrigger_every_step_project_runbook_webhook_trigger_secret" {
  count              = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? 0 : 1}"
  space_id           = "${trimspace(var.octopus_space_id)}"
  name               = "Runbook Webhook Trigger Secret"
  description        = "An example of a runbook webhook trigger that requires a secret to trigger."
  project_id         = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? data.octopusdeploy_projects.project_every_step_project.projects[0].id : octopusdeploy_project.project_every_step_project[0].id}"
  secret             = "${var.projecttrigger_every_step_project_runbook_webhook_trigger_secret_secret}"
  tenant_ids         = []
  run_runbook_action = { runbook_id = "${length(data.octopusdeploy_projects.project_every_step_project.projects) != 0 ? null : octopusdeploy_runbook.runbook_every_step_project_runbook_scoped_to_projects_lifecycle_environment[0].id}", target_environment_ids = ["${length(data.octopusdeploy_environments.environment_test.environments) != 0 ? data.octopusdeploy_environments.environment_test.environments[0].id : octopusdeploy_environment.environment_test[0].id}"] }
  lifecycle {
    ignore_changes  = [secret]
    prevent_destroy = true
  }
}
variable "projecttrigger_every_step_project_runbook_webhook_trigger_secret_secret" {
  type        = string
  nullable    = false
  sensitive   = true
  description = "The secret used to authenticate calls to the webhook trigger \"Runbook Webhook Trigger Secret\""
  default     = "Change Me!"
}
...
resource "octopusdeploy_project" "project_every_step_project" {
  ...
  is_disabled                          = false
  is_version_controlled                = false
  lifecycle_id                         = "${length(data.octopusdeploy_lifecycles.lifecycle_blue_green_lifecycle.lifecycles) != 0 ? data.octopusdeploy_lifecycles.lifecycle_blue_green_lifecycle.lifecycles[0].id : octopusdeploy_lifecycle.lifecycle_blue_green_lifecycle[0].id}"
