...
resource "octopusdeploy_project_deployment_target_trigger" "projecttrigger_every_step_project_deployment_target_trigger" {
  count            = "${length(data.octopusdeploy_projects.Every Step Project.projects) != 0 ? 0 : 1}"
...
resource "octopusdeploy_project_scheduled_trigger" "projecttrigger_every_step_project_scheduled_trigger" {
  ...
  run_runbook_action {
    ...
    runbook_id             = "${length(data.octopusdeploy_projects.Every Step Project.projects) != 0 ? null : octopusdeploy_runbook.runbook_every_step_project_example_runbook[0].id}"
  }
  lifecycle {
    prevent_destroy = true
  }
