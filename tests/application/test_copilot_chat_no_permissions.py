...
class CopilotChatNoDefaultsTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        try:
            ...
            service_account = next((user for user in users if user["Username"] == "bsmith"), None)
            api_key = create_unlimited_api_key(service_account["Id"], Octopus_Api_Key, Octopus_Url)
            ...
            delete_default_values(os.environ["TEST_GH_USER"],
                                  os.environ["AzureWebJobsStorage"])
    ...
    def test_help(self):
        ...
        self.assertTrue("https://octopus.com/docs/administration/copilot" in response_text,
