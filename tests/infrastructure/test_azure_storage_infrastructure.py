...
from infrastructure.users import (
    ...
    get_users_details,
    delete_all_user_details,
    delete_old_user_details,
    database_connection_test,
    delete_default_values,
    enable_feature_flag_for_user,
    is_feature_flagged_for_user,
    disable_feature_flag_for_user,
    enable_feature_flag_for_group,
    is_feature_flagged_for_group,
    disable_feature_flag_for_group,
    enable_feature_flag_for_all,
    is_feature_flagged_for_all,
    disable_feature_flag_for_all,
    delete_user_details,
    save_users_codefresh_details_from_login,
    get_users_codefresh_details,
    delete_all_codefresh_user_details,
...
class UsersTest(unittest.TestCase):
    ...
    def test_feature_flag_user(self):
        ...
        self.assertTrue(
            is_feature_flagged_for_user("testfeature", "testuser", connection_string)
        )
        self.assertFalse(
            is_feature_flagged_for_user("testfeature", "anotheruser", connection_string)
        )

        self.assertTrue(
            is_feature_enabled_for_github_user(
                "testfeature", "testuser", [], connection_string
            )
        )
        self.assertFalse(
            is_feature_enabled_for_github_user(
                "testfeature", "anotheruser", [], connection_string
            )
        )
        ...
        self.assertFalse(
            is_feature_flagged_for_user("unsetfeature", "testuser", connection_string)
        )

    def test_feature_flag_group(self):
        enable_feature_flag_for_group(
            "testgroupfeature", "testgroup", connection_string
        )
        self.assertTrue(
            is_feature_flagged_for_group(
                "testgroupfeature", "testgroup", connection_string
        ...
        self.assertFalse(
            is_feature_flagged_for_group(
                "testgroupfeature", "anothergroup", connection_string
            )
        )

        self.assertTrue(
            is_feature_enabled_for_github_user(
                "testgroupfeature", "testuser", ["testgroup"], connection_string
            )
        )
        self.assertFalse(
            is_feature_enabled_for_github_user(
                "testgroupfeature", "testuser", ["anothergroup"], connection_string
            )
        )
        ...
        self.assertFalse(
            is_feature_flagged_for_group("unsetfeature", "testgroup", connection_string)
        )
    ...
    def test_feature_flag_global(self):
        ...
        self.assertTrue(
            is_feature_enabled_for_github_user(
                "testallfeature", "testuser", ["testgroup"], connection_string
            )
        )

        disable_feature_flag_for_all("testallfeature", connection_string)
        self.assertFalse(
            is_feature_flagged_for_all("testallfeature", connection_string)
        )
    ...
    def test_login_invalid_url(self):
        with self.assertRaises(ValueError):
            save_users_octopus_url_from_login(
                "12345", "blah", "API-ABCDEFG", "password", "salt", connection_string
            )
    ...
    def test_codefresh_login_invalid_token(self):
        with self.assertRaises(ValueError):
            save_users_codefresh_details_from_login(
                "12345", "invalid", "password", "salt", connection_string
            )
