...
class MockRequests(unittest.TestCase):
    ...
    @unittest.skip("Known flaky test")
    ...
    def test_create_lambda_project(self):
        ...
        query = 'Create a AWS Lambda project called "OAM API 3" in the project group "AWS", replacing the account "AWS OIDC" with the AWS Account with the name "My AWS Account 3" and with an access key of "ABCDEFGHIJKLMNOPQRST".'
        function = llm_tool_query(query, build_mock_test_tools(query))
    ...
    @retry((AssertionError, RateLimitError), tries=3, delay=2)
    def test_create_project_with_lifecycle(self):
        """
        ...
        query = 'Create a script project called "01. My Test Project" and then create a lifecycle called "My Lifecycle" with environments "Dev", "QA", and "Prod".'
        function = llm_tool_query(query, build_mock_test_tools(query))
