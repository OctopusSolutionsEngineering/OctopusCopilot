...
def deploy_release(
    ...
    space = get_by_name(
        "{0}/spaces/all".format(octopus_server_uri), headers, space_name
    )
    project = get_by_name(
    ...
    releases = get_octopus_resource(
        "{0}/{1}/projects/{2}/releases".format(
            octopus_server_uri, space["Id"], project["Id"]
        ),
        headers,
    )
    release = next(
        (x for x in releases["Items"] if x["Version"] == release_version), None
    )
    environment = get_by_name(
        "{0}/{1}/environments/all".format(octopus_server_uri, space["Id"]),
        headers,
        environment_name,
    )

    deployment = {"ReleaseId": release["Id"], "EnvironmentId": environment["Id"]}
