...
class BuildUrl(unittest.TestCase):
    def test_url_partial_name(self):
        url = build_unredirected_url(
            "https://example.org", "/path", dict(partialname="value")
        )
    ...
    def test_url_no_query(self):
        url = build_unredirected_url("https://example.org/", "hi", None)
        self.assertEqual("https://example.org/hi", url)

    def test_empty_url(self):
        with self.assertRaises(ValueError):
            build_unredirected_url("", "/path", {"query": "value"})
