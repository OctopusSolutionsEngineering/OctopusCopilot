...
class BuildUrl(unittest.TestCase):
    def test_url_partial_name(self):
        url, headers = build_url(
            "https://example.org", "API-123", "/path", dict(partialname="value")
        )
    ...
    def test_url_no_query(self):
        url, headers = build_url("https://example.org/", "API-123", "hi", None)
        self.assertEqual("https://example.org/hi", url)

    def test_empty_url(self):
        with self.assertRaises(ValueError):
            build_url("", "API-123", "/path", {"query": "value"})
