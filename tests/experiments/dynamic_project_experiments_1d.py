    ...
    projects = list(
        map(
            lambda x: (
                ...
                x["Description"],
                (
                    x.get("VersioningStrategy") if x.get("VersioningStrategy") else {}
                ).get("Template"),
            ),
            projects,
        )
    ...
    def test_projects(self):
        ...
        for name, id, description, template in test_cases:
            with self.subTest(f"{name} - {id} - {description} - {template}"):
                ...
                query = (
                    f'What is the ID, description, and versioning strategy template of the project "{name}" '
                ...
                self.assertTrue(
                    id in result,
                    f'Expected "{id}" for Project {name} in result:\n{result}',
                )
                if template:
                    self.assertTrue(
                        template in result,
