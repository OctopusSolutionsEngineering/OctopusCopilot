...
from infrastructure.octopus import get_feeds, get_space_id_and_name_from_name
...
def get_test_cases(limit=0):
    ...
    tenants = list(
        map(
            lambda x: (x.get("Name"), x.get("Id"), x.get("FeedUri"), x.get("FeedType")),
            tenants,
        )
    )
    ...
    def test_feeds(self):
        ...
        for name, id, uri, type in test_cases:
            ...
            with self.subTest(f"{name} - {id} - {uri}"):
                ...
                self.assertTrue(
                    id in result,
                    f'Expected "{id}" for Feed {name} in result:\n{result}',
                )
                ...
                self.assertTrue(
                    feed_type in result or type in result,
                ...
                if uri and uri.strip():
                    self.assertTrue(
                        uri in result,
                        f'Expected "{uri}" for Feed {name} in result:\n{result}',
                    )
